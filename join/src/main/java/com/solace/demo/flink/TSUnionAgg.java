...
/**
 *  Approach: merge all input streams via .union() call, then run a key-wise aggregator on all messages. It will effectively join the input streams.
 */
public class TSUnionAgg {
    private static final Logger LOG = LoggerFactory.getLogger(TSUnionAgg.class);
    ...
    public void run() throws Exception {
        ...
        DataStream<TimeRecordWithKey> joinedInput = stream1.union(stream2).union(stream3);
    ...
    private PSPSink<TimeRecordWithKey> getPSPSink(StreamExecutionEnvironment env) {
        ...
        PSPSink<TimeRecordWithKey> psink = new PSPSink<> (
            ...
            new TimeRecordWithKeySerializationSchema(),
                (TimeRecordWithKey rk) -> String.format("solace/samples/union/%d", rk.getKey()));
        return psink;
    }

    public static void main(String[] args) {
        TSUnionAgg theJob = new TSUnionAgg();
